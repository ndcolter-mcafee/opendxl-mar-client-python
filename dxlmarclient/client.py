...
class MarClient(Client):
    ...
    def search(self, projections, conditions=None):
        ...
        Once the search has completed a :class:`ResultsContext` object is
        returned which is used to access the search results.

        .. note::

            **Client Authorization**

            The OpenDXL Python client invoking this method must have permission
        ...
        Execution of a MAR search requires a ``list`` of `projections` and an
        optional ``dictionary`` containing the search `conditions`.

        **Projections**

            `Projections` are used to describe the information to collect in the
            search. Each `projection` consists of a `collector` name and a list
            of `output names` from the `collector`. For example, the
            ``Processes`` `collector` includes `output names` such as ``name``,
            ``sha1``, ``md5``, etc.

            For a complete list of `collectors` and their associated `output
            names` refer to the `McAfee Active Response Product Guide`.
            ...
            * ``outputs``: An array of `output names` of the `collector` to project
            ...
            The python ``list`` below is equivalent to the `projections` within
            the following textual search:
        ...
        **Conditions**
            ...
            `Conditions` are used to restrict which items are included in the
            search results. For example, a search that collects process-related
            information could be limited to those processes which match a
            specified name.
            ...
            The python ``dictionary`` below is equivalent to the `conditions`
            within the following textual search:
...
class ResultsContext(object):
    ...
    def __init__(self, mar_client, search_id, result_count, error_count,
                 host_count, subscribed_host_count):
    ...
    def get_results(self, offset=0, limit=20, text_filter="", sort_by="count",
                    sort_direction=SortConstants.DESC):
        """
        ...
        **Results**
            ...
            * ``output``: The search result data where each key is composed of
              ``<CollectorName>|<OutputName>`` and the value that correspond to
              that `collector` and `output name`.

            The python ``dictionary`` below is an example of a result that would
            be returned from the following textual search:
        ...
        **Example Usage**
        ...
        :param limit: (optional) The maximum number of items to return in the
            results. Default value: ``20``
        :param text_filter: (optional) A text based filter to limit the results
            (this can be any string)
        :param sort_by: (optional) The field that will be used to sort the results.
            Default value: ``count``
        :param sort_direction: (optional) values: ascending ``asc`` or
            descending ``desc`` (String). Default value: ``desc``
